...
def fornecedores_remessas_atualizar(request, id):
    try:
        if request.method == 'POST':
            ...
            print('2')
            form.preencher_form({
                'componentes': encomendas_selected_arr,
				...
				'fatura_descricao': remessa['fatura_fornecedor'][0]['descricao']
            })
            print('3')
...
def make_array_of_encomendas_ids(data_encomenda):
    ...
    for detalhe in data_encomenda.get('detalhe_remessa_fornecedor', []):
        componentes_array = detalhe.get('detalhe_encomenda_fornecedor', [])
